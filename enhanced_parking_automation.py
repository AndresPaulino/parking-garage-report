...
class EnhancedParkingAutomation:
    ...
    async def process_all_reports(self, year: int, month: int, 
        ...
        logger.info(f"Processing {len(accounts)} accounts for {len(date_pairs)} days = {total_operations} operations")
        ...
        for account_value, account_name in accounts:
            # Check if this account was already processed (for resume)
        ...
        if not resume or len(progress.get('completed_accounts', [])) == len(accounts):
            self.clear_progress()
